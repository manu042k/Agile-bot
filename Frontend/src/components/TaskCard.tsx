...
const TaskCard = () => {
  ...
  const renderField = (label: string, name: keyof Task, type: 'text' | 'textarea' | 'select', options?: string[]) => (
    <div className="space-y-2">
      ...
      {isEditing ? (
        type === 'select' ? (
          <Select onValueChange={handleSelectChange(name)} defaultValue={typeof task[name] === 'string' ? task[name] : undefined}>
          ...
          <Textarea id={name} name={name} value={task[name]} onChange={handleInputChange} className="w-full" />
        ) : (
          <Input type="text" id={name} name={name} value={task[name]} onChange={handleInputChange} className="w-full" />
        )
      ) : (
        ...
        <Dialog>
          ...
          <DialogContent className="sm:max-w-xl h-auto max-h-[90vh] overflow-auto p-4">
            <DialogHeader>
              ...
              <DialogDescription>
                {isEditing ? "Modify task details." : "View and edit task information."}
              </DialogDescription>
            </DialogHeader>
            <Separator></Separator>
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-4">
                {renderField('Status', 'status', 'select', ['Not Started', 'In Progress', 'Completed'])}
                {renderField('Priority', 'priority', 'select', ['Low', 'Medium', 'High'])}
                {renderField('Size', 'size', 'select', ['Small', 'Medium', 'Large'])}
