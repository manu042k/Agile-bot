...
class LLMOrchestrator:
    ...
    def generate_tasks_from_document(
        ...
        # Log all requirement IDs found
        req_ids = [chunk['metadata']['requirement_id'] for chunk in chunks]
        logger.info(f"Requirement IDs found: {', '.join(sorted(set(req_ids)))}")
        ...
        for chunk in chunks:
            ...
            logger.info(f"Generating tasks for {req_id}... (chunk {chunks_processed + 1}/{len(chunks)})")
            chunks_processed += 1
            ...
            logger.info(f"  LLM generated {len(llm_tasks)} tasks for {req_id}")
        ...
        logger.info(f"  Tasks generated by LLM: {tasks_generated}")
        logger.info(f"  Tasks filtered (hallucinations): {tasks_filtered}")
        logger.info(f"  Final tasks created: {len(generated_tasks)}")
        logger.info(f"=" * 60)
