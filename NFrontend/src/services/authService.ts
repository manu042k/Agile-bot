...
const authService = {
  ...
  async syncWithDjango(googleId: string, email: string): Promise<void> {
    try {
      const response = await axios.post(
        `${URLS.BASE_URL}${URLS.GOOGLE_SYNC}`,
        { google_id: googleId, email },
        { withCredentials: true } // Important for session cookies
      );
      console.log("[Django Sync] Successfully synced with Django:", response.data);
      return response.data;
    } catch (error: any) {
